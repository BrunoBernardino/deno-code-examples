// NOTE: If you'd like to use a migrations tool with more features (like JSON migrations), check out https://deno.land/x/migrate
...
async function getExecutedMigrations() {
  const executedMigrations = new Set(
    Array.from(
      (await db.query<Migration>(sql`SELECT * FROM "migrations" ORDER BY "name" ASC`)).map((migration) =>
        migration.name
      ),
    ),
  );
...
async function runMigrations(missingMigrations: Set<string>) {
  for (const missingMigration of missingMigrations) {
    ...
    try {
      ...
      await db.query(sql`INSERT INTO "public"."migrations" ("name", "executed_at") VALUES ($1, NOW())`, [
        missingMigration,
      ]);
