...
function permissioned(handler: RequestHandler, accessMode: AccessMode) {
  return ({ request, match, user, session }: RequestHandlerParams) => {
    if (accessMode !== 'public') {
      if (!user) {
        const url = new URL(request.url);
        const redirectTo = encodeURIComponent(`${url.pathname}${url.search}`);
        return new Response('Redirect', { status: 302, headers: { 'Location': `/?redirectTo=${redirectTo}` } });
      }
    }
    ...
    return handler({ request, match, user, session });
  };
}
